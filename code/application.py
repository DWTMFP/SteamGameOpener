...
class window():
        ...
        for i in range(len(self.appids)):
            ...
            if not img_i_used: #if no img was used for this appid, add the game, without it
                try:
                    ...
                    if os.listdir(self.helper.CURRENT_DIRECTORY).__contains__("images"):
    ...
    def scroll(self, event):
        self.choose_game_canvas.configure(scrollregion=self.choose_game_canvas.bbox("all"), height = self.user_config.HEIGHT_SCROLLBAR)
...
if __name__ == "__main__":
    ...
    if args.cfg_file != None: #a config file was parsed
        try:
            main(importlib.import_module(args.cfg_file.removesuffix(".py"))) #try to import the config file (with removed .py) and start the application with it
        except ImportError:
        ...
        try:
            ...
            print("Something went wrong, when trying to import user_config.py\nMake sure it is installed, named correctly and placed in the same folder as this file.")
        main(user_config)
